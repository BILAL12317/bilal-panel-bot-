...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    product = product_names.get(query.data, "Unknown Product")

    await query.edit_message_text(
        f"{product}\n\n"
        "💰 Contact admin for price.\n\n"
        "After payment, press 💳 Payment and send your screenshot."
    )
...
async def receive_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    app.add_handler(
        MessageHandler(filters.Regex(r"^🛒 Products$"), products)
    )
    app.add_handler(CallbackQueryHandler(button))
    app.add_handler(
        MessageHandler(filters.Regex(r"^💳 Payment$"), payment)
    )
    app.add_handler(
        MessageHandler(filters.Regex(r"^📞 Contact$"), contact)
    )
    app.add_handler(
